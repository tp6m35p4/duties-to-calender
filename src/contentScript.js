    ...
    try {
      if (current['type'] == 'F') {
        ...
        event[3] = current.portalCalendarDetailRosterInfoVoList.reduce(
          (acc, curr) => {
            let crewList = curr.portalCalendarDetailCrewInfoVos.reduce(
              (a, c) => {
                return (
                  a + `<li>${c.actingRank}: ${c.crewName} - ${c.crewId}</li>`
                );
              },
              ''
            );
...
async function getRoster(start, end) {
  const jwt = window.localStorage['PORTAL_it/portal_token'];
  const user = window.localStorage['PORTAL_it/portal_userCode'];
  let s = moment(start);
  let e = moment(end);
